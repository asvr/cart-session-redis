...
class MY_Session {
	...
	
	{
		...
		if ($this->sess_use_database === TRUE)
		{
			...
			$row = $redis->hgetall($sess_key);
			//var_dump($row);
			
			if ($row && $this->sess_match_ip == TRUE)
			{	
				if ( empty($row[3]) || $session['ip_address'] != $row[3] ) {
	...
	function sess_write()
	{
		...
		$write = $redis->hset($sess_key, 'last_activity', $this->userdata['last_activity']);
		$write = $redis->hset($sess_key, 'user_data', $custom_userdata);
			//$redis->expire($sess_key, $this->sess_expiration);
			//->execute();
	...
	function sess_create()
	{
		...
		if ($this->sess_use_database === TRUE)
		{
			...
			$redis->hset($sess_key, 'session_id', $this->userdata['session_id']);
			$redis->hset($sess_key, 'ip_address', $this->userdata['ip_address']);
			$redis->hset($sess_key, 'user_agent', $this->userdata['user_agent']);
			$redis->hset($sess_key, 'last_activity', $this->userdata['last_activity']);
			$redis->hset($sess_key, 'user_data', $this->userdata['user_data']);
			$redis->expire($sess_key, $this->sess_expiration);
		}
	...
	function sess_update()
	{
		...
		if ($this->sess_use_database === TRUE)
		{
			...
			$redis->hset($old_sess_key, 'last_activity', $this->now);
			$redis->hset($old_sess_key, 'session_id', $new_sessid);
			$redis->expire($old_sess_key, $this->sess_expiration);
			$redis->rename($old_sess_key, $new_sess_key);
